 ...
 * @author Enrico
 */
public class Motion extends EnvObjectLogic {
    ...
    protected void createTriggers() {
        ...
        Trigger clickedMotion = new Trigger();
        clickedMotion.setName("When " + this.getPojo().getName() + " is clicked");
        clickedMotion.setChannel("app.event.sensor.object.behavior.clicked");
        clickedMotion.getPayload().addStatement("object.name", this.getPojo().getName());
        clickedMotion.getPayload().addStatement("click", ObjectReceiveClick.SINGLE_CLICK);
        clickedMotion.setPersistence(false);        
                   
        triggerRepository.create(clickedMotion);
        
            
        
        
    }
    private static final Logger LOG = Logger.getLogger(Motion.class.getName());
}